...
raw_data <- read_csv("AAMV3rQa.csv")
...
gss <- gss %>%
  mutate(health = case_when(self_rated_health == "Excellent" ~ "Excellent",
                   self_rated_health == "Very good" ~ "Very good",
                   self_rated_health == "Good" ~ "Fair",
                   self_rated_health == "Fair" ~ "Fair",
                   self_rated_health == "Poor" ~ "Poor",
                   self_rated_health == "Don't know" ~ "Other",
                   self_rated_health == "Not stated" ~ "Other",
                   self_rated_health == "Refusal" ~ "Other"))
...
gss <- gss %>% 
  mutate(province = case_when(province == "Newfoundland and Labrador" ~ "NL",
...
census_data <- 
  reduced_data %>%
  count(age, sex, province, health) %>%
  group_by(age, sex, province, health) 
